...
{--
vk :: ExceptT BotError (ReaderT (Logger IO, Config) (StateT (Map.Map Integer Integer) IO)) ()
vk  = do
  logI "bot vk launched"
  --Config _ (Just(vkOpts _ token))  <- asks getter

getAccess = toAPI $ GetAccess()
...
getUpdates  o l t a_u  = do
      Config _ (Just(TelegramOpts _ _ btn))  <- asks getter
